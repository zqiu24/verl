...
from verl.utils.vllm import TensorLoRARequest, TensorOFTRequest, VLLMHijack
    ...
    def update_weights_from_ipc(self, peft_config: dict = None, base_sync_done=False, use_shm: bool = False):
        ...
        def _to_dict(cfg):
            ...
            if isinstance(d, dict):
                return d
            raise TypeError(f"Unsupported peft_config type for adapter sync: {type(cfg)}")

        peft_config = _to_dict(peft_config)

        # In async mode, make sure the old adapter is removed before adding the new one
        if peft_config and base_sync_done:
            peft_type = peft_config.get("peft_type", None)
            peft_type = peft_type.value if hasattr(peft_type, "value") else peft_type
            peft_type = str(peft_type).upper() if peft_type is not None else None
            if peft_type == "LORA":
                self.remove_lora(VLLM_LORA_INT_ID)
            elif peft_type == "OFT":
                self.remove_oft(VLLM_OFT_INT_ID)
    ...
    def _update_weights(self, weights: list[tuple[str, torch.Tensor]], peft_config: dict, base_sync_done: bool):
        if peft_config and base_sync_done:
            ...
            if peft_type == "LORA":
                ...
                self.add_oft(oft_request)
            else:
                raise ValueError(f"Unsupported peft_type for adapter sync: {peft_type}")
